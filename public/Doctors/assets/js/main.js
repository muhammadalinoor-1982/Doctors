...
//var pageUrl = window.location.href.substr(window.location.pathname.lastIndexOf("/") + 1);
    var pageUrl = window.location.pathname.substring(1);

    $('.side-header-menu a').each(function() {

        var page = $(this).attr("href").split("http://localhost/stflbd/");

        if (page[1] === pageUrl || $(this).attr("href") === '') {
